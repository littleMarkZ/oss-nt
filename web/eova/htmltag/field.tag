...
<%} else if(item.type == "文本域"){%>
	<div><#texts id="${item.en}" name="${item.en}" value="${item.value!item.defaulter}" isNoN="${item.is_required!}" placeholder="${item.placeholder!}" validator="${item.validator!}" style="width:468px;height:${item.height!20}px;" disable="${item.is_disable!}" /></div>
<%} else if(item.type == "编辑框"){%>
	<div><#edit id="${item.en}" name="${item.en}" value="${item.value!item.defaulter}" isNoN="${item.is_required!}" style="width: 468px;height:150px;margin-bottom: 5px;float: left;" disable="${item.is_disable!}" /></div>
